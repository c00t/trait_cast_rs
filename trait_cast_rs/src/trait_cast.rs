...
pub struct TraitcastTarget {
  target_type_id: FixedId,
  target_type_version: FixedVersion,
...
impl TraitcastTarget {
  ...
  #[must_use]
  pub const fn from<Src: TraitcastableTo<Target>, Target: 'static + ?Sized + FixedTypeId>() -> Self
  {
  ...
  #[must_use]
  pub const fn target_type_id(&self) -> FixedId {
    self.target_type_id
  }
  ...
  #[must_use]
  pub const fn target_type_version(&self) -> FixedVersion {
    ...
    self.traitcast_targets().iter().find(|possible| {
      ...
      possible.target_type_id == target
        && (possible.target_type_version == expected_version
          || possible.target_type_version.matches(&compatible_cmp))
    })
  }

  /// Returns the [`FixedId`] of the concrete type.
...
macro_rules! implement_with_markers {
  ($($(+)? $traits:ident)*) => {
    impl<Target: ?Sized + 'static + FixedTypeId + $($traits +)*> TraitcastableAnyInfra<Target> for dyn TraitcastableAny $(+ $traits)* {
